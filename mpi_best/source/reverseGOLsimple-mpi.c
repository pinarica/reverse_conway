...
int main(int argc, char *argv[]) {
	...
	for(int i=0; i<npop; i++) {
        ...
        // Idea: Take the best plate after each generation share it with all and repeat
        int best_thread = 0;
	int number = pop_fitness[best];
	int snumber = 100000000;
	char *best_plate = (char *) calloc((n+2)*(n+2), sizeof(char));
	char *best_plates[n_tasks];

	if(thread_id == 0){
	    best_plates[0] = population[best];
	    for(int i=1; i<n_tasks; i++){
		char *tbplate = (char *) calloc((n+2)*(n+2), sizeof(char));
		MPI_Recv(tbplate, ((n+2)*(n+2)), MPI_CHAR, i, i, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		MPI_Recv(&number, 1, MPI_INT, i, i, MPI_COMM_WORLD, MPI_STATUS_IGNORE);	
		best_plates[i] = tbplate;
	    ...
	    best_plate = best_plates[best_thread];
	...
	if(thread_id == 0){
            for(int i=1; i<n_tasks; i++){
                //char *tbplate = (char *) calloc((n+2)*(n+2), sizeof(char));
                MPI_Send(best_plates[best_thread], ((n+2)*(n+2)), MPI_CHAR, i, thread_id, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	    }
	}
	else{
	    char *tbplate = (char *) calloc((n+2)*(n+2), sizeof(char));
            MPI_Recv(tbplate, ((n+2)*(n+2)), MPI_CHAR, thread_id, thread_id, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	    population[best] = tbplate;
	}
	*/
